...
// MyBatis => 메소드 작성후 return selectList(SQL)
public class MovieDAO {
	...
	public List<MovieVO> movieListData(int page){
		...
		try {
			getConnection();
			int rowSize=12;
			int start=(rowSize*page)-(rowSize-1); // 해당 페이지의 시작 게시물 번호
			int end=rowSize*page; // 해당 페이지의 마지막 게시물 번호
			// rownum BETWEEN start AND end 쓰기위함
